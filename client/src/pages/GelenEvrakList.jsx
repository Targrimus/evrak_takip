...
const GelenEvrakList = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Append files to delete (only for update)
      ...
      if (isEditMode) {
        ...
        setEvraklar([res.data, ...evraklar]); // Add new to top
      }
      handleClose();
    } catch (err) {
              ...
              <thead className="bg-light">
                <tr>
                  ...
                  <th onClick={() => requestSort('documentDate')} style={{ cursor: 'pointer' }}>
                    Evrak Tarihi <SortIcon field="documentDate" />
                  </th>
                ...
                {filteredEvraklar.length === 0 ? (
                  ...
                  filteredEvraklar.map((evrak) => (
                    <tr key={evrak._id}>
                      ...
                      <td className="small text-muted">{new Date(evrak.createdAt).toLocaleDateString()}</td>
                      <td>
                      ...
                      <td>
                          ...
                          {evrak.status === 'COMPLETED' && evrak.dispatch?.dispatchDate && (
                              <div className="small text-muted mt-1" style={{fontSize: '0.75rem'}}>
                                  {new Date(evrak.dispatch.dispatchDate).toLocaleDateString()}
                              </div>
                          )}
                      </td>
                      <td>
